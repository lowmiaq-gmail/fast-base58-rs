...
- candidate contract tests: PASS (`13` on universal fallback; current native suite also passing).
- complete unfiltered upstream suite: PASS (`3` tests).
- isolated Oracle/Candidate differential: PASS (`seed=20260812`, `10,000` cases) for native and universal fallback.
- exact-commit local candidate set: one macOS arm64 abi3 wheel, one universal `py2.py3-none-any` fallback wheel, one sdist.
...
- fresh packaged fallback-wheel candidate/upstream/differential gates: PASS on Python 3.14.
...
- Python 2.7 fallback artifact execution;
